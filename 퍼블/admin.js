...
(function ($) {
  ...
  $('[data-dev-only]').on('click', '[data-mock]', function () {
    ...
    if (kind === 'qa_pick') { qaOpen(); return; }
    if (kind === 'qa_picked') {
      qaOpen();
      qaSetFile({ name: 'generated_qa.json', size: 48310 });
      return;
    }
    if (kind === 'qa_prev_over') { qaOpen(); qaOpenPreview(qaDummy('mixed'), 'generated_qa.json'); return; }
    if (kind === 'qa_prev_new') { qaOpen(); qaOpenPreview(qaDummy('allnew'), 'generated_qa.json'); return; }
    if (kind === 'qa_prev_zero') { qaOpen(); qaOpenPreview(qaDummy('zero'), 'generated_qa.json'); return; }
    if (kind === 'qa_running' || kind === 'qa_done') {
      qaOpen();
      qaOpenPreview(qaDummy('mixed'), 'generated_qa.json');
      $('#qaImportOverwrite').prop('checked', true).trigger('change');
      qaRun();
      return;
    }
    if (kind === 'login') { loginTries = 0; loginLocked = false; openLogin('default'); return; }
    if (kind === 'login_expired') { loginTries = 0; loginLocked = false; $loginId.val('portal.admin'); openLogin('expired'); return; }
    if (kind === 'login_fail') {
      loginTries = 1; loginLocked = false;
      $loginId.val('portal.admin'); openLogin('default'); $loginId.val('portal.admin');
      setLoginState('failed', LOGIN_ERR);
      return;
    }
    if (kind === 'login_locked') {
      loginTries = LOGIN_MAX_TRY; loginLocked = true;
      $loginId.val('portal.admin'); openLogin('default'); $loginId.val('portal.admin');
      setLoginState('locked', '로그인 시도가 ' + LOGIN_MAX_TRY + '회를 넘어 잠시 잠겼습니다. 5분 후 다시 시도해 주세요.');
      return;
    }
    if (kind === 'quick_err') {
      $('.admin_tab[data-tab="categories"]').trigger('click');
  ...
  /* ==========================================================================
     ...
     1) 파일 선택 → 2) 미리보기(서버 계산) → 3) 몇 건씩 나눠 전송 + 결과표
     · 들여온 QA 는 무조건 검수 대기입니다. 승인 상태로 가져오는 경로는 없습니다.
     · 되돌리기가 없어 2단계가 마지막 확인 지점이며, 덮어쓸 건수 > 0 이면 #confirmModal 을 한 번 더 띄웁니다.
     ========================================================================== */
  var QA_CHUNK = 3;               /* 한 요청에 보낼 건수 — 서버와 맞추세요 */
  var QA_ST_LABEL = { 'new': '새로', over: '덮음', skip: '건너뜀', added: '추가', fail: '실패' };
  var qaFile = null;
  var qaPreview = [];             /* 서버가 내려준 미리보기 목록 */
  var qaSummary = { 'new': 0, over: 0, skip: 0 };

  /* 목업 데이터 (개발 연동 시 삭제) */
  function qaDummy(kind) {
    var seeds = [
      ['권한그룹은 무엇을 선택해야 하나요?', 'API 등록 항목 설명', 4],
      ['API 등록 절차는 어떻게 되나요?', 'API 등록 절차', 5],
      ['템플릿을 수정하면 기존 API에도 반영되나요?', '템플릿 수정', 3],
      ['Swagger YAML로 한 번에 등록할 수 있나요?', 'OAS / Swagger 일괄 등록', 5],
      ['등록한 API가 목록에 안 보여요', 'API 등록 오류', 4],
      ['API Key는 어디서 발급하나요?', 'API Key 발급', 5],
      ['그룹을 삭제하면 하위 API는 어떻게 되나요?', '그룹 삭제', 4],
      ['Path와 Method는 어떻게 입력하나요?', 'API 등록 항목 설명', 3],
      ['배포 버튼이 활성화되지 않습니다', 'API 배포', 4],
      ['부서 기본 템플릿을 지정할 수 있나요?', '기본 템플릿 지정', 5],
      ['과금 정책은 어디서 확인하나요?', null, null],
      ['SSO 로그인이 실패합니다', 'SSO 로그인', 3]
    ];
    if (kind === 'zero') { return []; }
    return $.map(seeds, function (s, i) {
      var state = kind === 'allnew' ? 'new' : (i % 4 === 1 ? 'over' : i === 10 ? 'skip' : 'new');
      return { id: 'qa_' + i, question: s[0], category_label: s[1], score: s[2], state: state };
    });
  }
  ...
  function qaStage(stage) {
    ...
    if (stage === 'run' || stage === 'done') {
      $('#qaImportModal').find('.admin_import_stage[data-stage="run"]').prop('hidden', false);
    }
  }

  function qaSetFile(file) {
    qaFile = file || null;
    var ok = !!file && /\.json$/i.test(file.name);
    $('#qaImportDrop').toggleClass('is_invalid', !!file && !ok);
    $('#qaImportFileInfo').prop('hidden', !ok);
    if (ok) {
      $('#qaImportFileInfo').find('[data-bind="file_name"]').text(file.name);
      $('#qaImportFileInfo').find('[data-bind="file_size"]').text((file.size / 1024).toFixed(1) + ' KB');
      setState($('#qaImportState'), null);
    } else if (file) {
      setState($('#qaImportState'), 'err', '.json 파일 1개만 올릴 수 있습니다');
    }
    $('#qaImportCheckBtn').prop('disabled', !ok);
  }

  function qaRenderPreview() {
    var overwrite = $('#qaImportOverwrite').prop('checked');
    var $tb = $('#qaImportPreviewBody').empty();
    var sum = { 'new': 0, over: 0, skip: 0 };
    $.each(qaPreview, function (i, q) {
      /* 체크박스를 끄면 이미 있는 것(over)은 전부 건너뜀으로 바뀝니다 */
      var state = (q.state === 'over' && !overwrite) ? 'skip' : q.state;
      sum[state]++;
      var $r = tpl('tpl_qa_prev_row').attr('data-qa-id', q.id);
      $r.find('[data-bind="state"]').text(QA_ST_LABEL[state]).addClass('is_' + state);
      $r.find('[data-bind="question"]').text(q.question);
      $r.find('[data-bind="category_label"]').text(q.category_label || '(주제 없음)');
      $r.find('[data-bind="score"]').text(q.score == null ? '—' : q.score);
      $tb.append($r);
    });
    qaSummary = sum;
    $('.admin_import_sum').find('[data-bind="new_count"]').text(sum['new']);
    $('.admin_import_sum').find('[data-bind="over_count"]').text(sum.over);
    $('.admin_import_sum').find('[data-bind="skip_count"]').text(sum.skip);
    var target = sum['new'] + sum.over;
    $('#qaImportPreviewBody').closest('.admin_table_wrap').toggle(qaPreview.length > 0);
    $('#qaImportPreviewEmpty').prop('hidden', qaPreview.length > 0);
    $('#qaImportStartBtn').prop('disabled', target === 0);
  }

  function qaOpenPreview(list, fileName) {
    qaPreview = list;
    $('.admin_import_stage[data-stage="preview"]').find('[data-bind="file_name"]').text(fileName || (qaFile ? qaFile.name : 'generated_qa.json'));
    $('.admin_import_stage[data-stage="preview"]').find('[data-bind="total_count"]').text('· ' + list.length + '건');
    $('#qaImportOverwrite').prop('checked', false);      /* 끈 상태가 기본 */
    qaRenderPreview();
    qaStage('preview');
  }
  ...
  function qaRun() {
    var overwrite = $('#qaImportOverwrite').prop('checked');
    var targets = $.grep(qaPreview, function (q) {
      return q.state === 'new' || (q.state === 'over' && overwrite);
    });
    qaStage('run');
    $('#qaImportResultBody').empty();
    $('#qaImportProgress').prop('hidden', false);
    $('#qaImportProgressFill').css('width', '0%');
    $('#qaImportProgressNum').text('0 / ' + targets.length);
    $('#qaImportProgressLabel').text('가져오는 중…');
    setState($('#qaImportState'), 'busy', '가져오는 중…');
    $('#qaImportStartBtn').prop('disabled', true);

    var sent = 0;
    (function step() {
      /* 개발 연동 지점: 몇 건씩 나눠 POST (한 요청에 다 담으면 진행을 보여줄 수 없고
         타임아웃 시 무엇이 들어갔는지 알 수 없습니다) */
      var chunk = targets.slice(sent, sent + QA_CHUNK);
      if (!chunk.length) {
        $('#qaImportProgressLabel').text('완료되었습니다');
        setState($('#qaImportState'), 'ok', targets.length + '건을 검수 대기로 가져왔습니다');
        qaStage('done');
        toast('ok', targets.length + '건을 검수 대기로 가져왔습니다.');
        return;
      }
      api('importQa', { items: chunk, overwrite: overwrite }, function () {
        $.each(chunk, function (i, q) {
          var fail = !q.category_label;              /* 목업: 주제 없으면 실패 */
          var $r = tpl('tpl_qa_result_row').attr('data-qa-id', q.id);
          $r.find('[data-bind="question"]').text(q.question);
          $r.find('[data-bind="result"]')
            .text(fail ? QA_ST_LABEL.fail : q.state === 'over' ? QA_ST_LABEL.over : QA_ST_LABEL.added)
            .addClass(fail ? 'is_fail' : q.state === 'over' ? 'is_over' : 'is_added');
          $r.find('[data-bind="note"]').text(fail ? '주제 없음' : '검수 대기');
          $('#qaImportResultBody').append($r);
        });
        sent += chunk.length;
        $('#qaImportProgressFill').css('width', (sent / targets.length * 100) + '%');
        $('#qaImportProgressNum').text(sent + ' / ' + targets.length);
        step();
      });
    })();
  }
  ...
  function qaOpen() {
    ...
    $('#qaImportCheckBtn').prop('disabled', true);
  ...
  $('#qaImportFile').on('change', function () { qaSetFile(this.files && this.files[0]); });
  $('#qaImportFileClear').on('click', function () {
    $('#qaImportFile').val('');
    qaSetFile(null);
  });
  $('#qaImportDrop').on('dragover', function (e) {
    e.preventDefault();
    $(this).addClass('is_over');
  }).on('dragleave drop', function () { $(this).removeClass('is_over'); });
  $('#qaImportDrop').on('drop', function (e) {
    e.preventDefault();
    var dt = e.originalEvent && e.originalEvent.dataTransfer;
    qaSetFile(dt && dt.files && dt.files[0]);
  });

  $('#qaImportCheckBtn').on('click', function () {
    setState($('#qaImportState'), 'busy', '내용을 확인하는 중…');
    /* 미리보기 숫자는 서버가 계산해서 내려줍니다 — 화면이 세지 않습니다 */
    api('previewQa', { file: qaFile ? qaFile.name : null }, function () {
      setState($('#qaImportState'), null);
      qaOpenPreview(qaDummy('mixed'));
    });
  });

  $('#qaImportOverwrite').on('change', qaRenderPreview);

  $('#qaImportStartBtn').on('click', function () {
    if (qaSummary.over > 0) {
      askConfirm({
        title: '덮어쓰기를 포함해 가져올까요?',
        message: '덮어쓸 항목 ' + qaSummary.over + '건',
        desc: '가져오기는 되돌릴 수 없습니다. 적중 횟수와 사용자 신고는 서버 값이 유지되며, 모든 항목은 검수 대기로 들어옵니다.',
        okLabel: '가져오기'
      }, qaRun);
      return;
    }
  ...
  $('#qaImportGotoBtn').on('click', function () {
    ...
    /* 이식본에서는 사이드바 ④ 검수 화면으로 이동합니다 */
    var tab = $(this).attr('data-goto-tab');
    var $t = $('.admin_tab[data-tab="' + tab + '"]');
    if ($t.length) { $t.trigger('click'); } else { toast('ok', '검수 화면으로 이동합니다.'); }
  });

  /* ==========================================================================
     로그인 모달 (요청서 03 — 02의 login.html 대체)
     · ESC / 배경 클릭 / ✕ 로 닫히지 않습니다.
     · 처음 접속(default) / 세션 만료(expired) 를 한 모달로 처리하며,
       만료 시 배경(편집 중인 내용)은 지우지 않습니다.
     · 아이디·비밀번호는 화면에 저장하지 않습니다(localStorage 포함).
       인증 판단은 전적으로 서버(POST /api/admin/login)에 있습니다.
     ========================================================================== */
  var LOGIN_MAX_TRY = 5;          /* 서버 정책과 맞춰야 하는 값 */
  var loginTries = 0;
  ...
  var LOGIN_LEAD = {
    'default': 'API Manager 도우미 관리자 설정입니다. 사내 계정으로 로그인해 주세요.',
    expired: '보안을 위해 세션이 종료되었습니다. 다시 로그인하면 작업하던 내용을 이어서 저장할 수 있습니다.'
  };
  /* 오류 문구는 항상 한 문장 — 아이디 존재 여부를 알려주지 않습니다 */
  var LOGIN_ERR = '아이디 또는 비밀번호가 올바르지 않습니다.';

  function setLoginState(state, message) {
    $login.attr('data-login-state', state);
    var busy = state === 'submitting';
    $loginSubmit.prop('disabled', busy || loginLocked || !loginCanSubmit());
    $('#loginSubmit').find('.admin_login_submit_label').text(busy ? '로그인 중…' : loginLocked ? '잠시 후 다시 시도' : '로그인');
    if (message) {
      $('#loginErr').prop('hidden', false).find('[data-bind="error_message"]').text(message);
    } else if (state !== 'failed' && state !== 'locked') {
      $('#loginErr').prop('hidden', true);
    }
  }

  function loginCanSubmit() {
    return !!$.trim($loginId.val()) && !!$loginPw.val() && !loginLocked;
  }

  function openLogin(kind) {
    var expired = kind === 'expired';
    $login.prop('hidden', false).addClass('is_open').toggleClass('is_expired', expired);
    $('#loginLead').text(expired ? LOGIN_LEAD.expired : LOGIN_LEAD['default']);
    /* 만료 시엔 아이디를 남겨 두고 비밀번호만 비웁니다 */
    if (!expired) { $loginId.val(''); }
    $loginPw.val('');
    $('#loginCaps').prop('hidden', true);
    setLoginState('default');
    setTimeout(function () {
      ($.trim($loginId.val()) ? $loginPw : $loginId).trigger('focus');
    }, 30);
  }

  function closeLogin() {
    $login.removeClass('is_open is_expired').prop('hidden', true);
    /* 화면에 자격증명을 남기지 않습니다 */
    $loginPw.val('');
    setLoginState('default');
  }

  /* 입력 중 — 오류 문구를 걷고 버튼 활성화 */
  $('#loginId, #loginPw').on('input', function () {
    if (loginLocked) { return; }
    $('#loginErr').prop('hidden', true);
    setLoginState($.trim($loginId.val()) || $loginPw.val() ? 'typing' : 'default');
  });

  /* Caps Lock 안내 */
  $('#loginPw, #loginId').on('keydown keyup', function (e) {
    var ev = e.originalEvent;
    if (!ev || typeof ev.getModifierState !== 'function') { return; }
    $('#loginCaps').prop('hidden', !ev.getModifierState('CapsLock'));
  });

  /* 비밀번호 표시 토글 */
  $('#loginPwToggle').on('click', function () {
    var on = $(this).attr('aria-pressed') !== 'true';
    $(this).attr('aria-pressed', on ? 'true' : 'false')
      .attr('aria-label', on ? '비밀번호 숨기기' : '비밀번호 표시');
    $loginPw.attr('type', on ? 'text' : 'password').trigger('focus');
  });
  ...
  $('#loginForm').on('submit', function (e) {
    ...
    /* 개발 연동 지점: POST /api/admin/login (세션 쿠키)
       $.ajax({ url: '/api/admin/login', type: 'POST', contentType: 'application/json',
         data: JSON.stringify({ username: $loginId.val(), password: $loginPw.val() }) })
         .done(function (res) { ... }).fail(function (xhr) { ... 401 / 423 ... }); */
    var ok = $loginPw.val() === 'admin';       /* 목업 판정 — 실제 판단은 서버 */
    setTimeout(function () {
      if (ok) {
        loginTries = 0;
        $('#adminSessionId').text($.trim($loginId.val()));
        closeLogin();
        toast('ok', '로그인되었습니다. 작업하던 내용을 이어서 저장할 수 있습니다.');
        return;
      }
      loginTries++;
      /* 실패해도 아이디는 남기고 비밀번호만 지웁니다 */
      $loginPw.val('').attr('type', 'password');
      $('#loginPwToggle').attr('aria-pressed', 'false').attr('aria-label', '비밀번호 표시');
      if (loginTries >= LOGIN_MAX_TRY) {
        loginLocked = true;
        setLoginState('locked', '로그인 시도가 ' + LOGIN_MAX_TRY + '회를 넘어 잠시 잠겼습니다. 5분 후 다시 시도해 주세요.');
        return;
      }
  ...
  /* 포커스 순환 — 모달 밖으로 탭이 빠져나가지 않게 */
  $login.on('keydown', function (e) {
    if (e.key === 'Escape') { e.preventDefault(); e.stopPropagation(); return; }   /* 닫히지 않음 */
    if (e.key !== 'Tab') { return; }
    var $f = $login.find('input:visible:not([disabled]), button:visible:not([disabled])');
    if (!$f.length) { return; }
  ...
  /* 배경 클릭으로 닫히지 않도록 공통 핸들러보다 먼저 차단 */
  $login.on('click mousedown', function (e) { e.stopPropagation(); });

  $('#btnLogout').on('click', function () {
    /* 개발 연동 지점: POST /api/admin/logout */
    api('logout', {}, function () {
      $('#adminSessionId').text('—');
      openLogin('default');
      toast('ok', '로그아웃되었습니다.');
  ...
  renderQuick();
  renderZones();
  $('#adminSessionId').text('portal.admin');
  renderAnKpi();
  renderAnCharts();
  renderClusters();
})(jQuery);
